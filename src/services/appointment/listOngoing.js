...
const appointmentListOngoing = async ({ params, token, maxDays = 14 }) => {
  try {
    let startDate
    let endDate
    ...
    const { data: response } = await axios.post(
      `${process.env.NEXT_PUBLIC_BASE_URL_APPOINTMENT_SERVICE}/v1/consultation/ongoing`,
      params,
      {
        headers: {
...
export default appointmentListOngoing
